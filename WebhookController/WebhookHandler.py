...
#            )
    ...
    def perform_makeguess(self, cowbull_url=None, parameters=None, contexts=None):
        ...
        key = [n["name"]["key"] for n in _contexts if n["name"] == "key"][0]
        digits_required = [n["name"]["digits"] for n in _contexts if n["name"] == "digits"][0]
        guesses = [n["name"]["guesses"] for n in _contexts if n["name"] == "guesses"][0]
